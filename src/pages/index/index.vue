    ...
    <view class="content">
        <view class="text-area">
            <view class="together">
                ...
                <cover-view v-if="types" class="together-choose">
                    <cover-view
            ...
            <view class="together-content">
                <textarea
                    placeholder="请输入任务内容,不超过40字"
                    placeholder-style="color:#333;font-size:14px;"
                    maxlength="40"
                    @blur="yueContent"
                />
                <view v-if="addImg" class="picture">
                    <view v-for="(item,index) in imgList" :key="index" class="imgList">
                        <image :src="item" />
                    </view>
                    <view class="addpicture" @click="chooseAlbum"></view>
                </view>
                <view class="camera">
                    <image src="./../../static/image/cermea.jpg" @click="addImgs" />
                </view>
            </view>
            ...
            <view v-if="showChooseTime">
                ...
                <view class="frequency">
                    <view
                        v-for="(item,index) in AgreementCount"
                        :key="index"
                        class="frequency1"
                        :class="frequencyKey===index?'active':''"
                ...
                <view v-if="showCryl">
                    ...
                    <view class="frequency">
                        ...
                        <view
                            ...
                            @click="frequency(4)"
                        >
                            <input placeholder="请输入固定次数" />
                        </view>
                    </view>
                </view>
            </view>

            <view v-if="showChooseTime" class="chooseTime">
        ...
        <view v-if="showDetail" class="detail">
    ...
    methods: {
        ...
        getAgreementList() {
            agreementList().then(res => {
                this.typeList = this.filter(res);
            });
        },
        // 获取约定次数类型
        getAgreementCountList() {
            agreementCountList().then(res => {
                this.AgreementCount = this.filter(res);
            });
        },
        // 约定周期类型
        getAgreementCycle() {
            agreementCycle().then(res => {
                this.cycleList = this.filter(res);
            });
        },
        // 约定周期时间
        getAgreementCycleTime() {
            agreementCycleTime().then(res => {
                this.array = this.filter(res);
            });
        },
        // 选择约定次数
        frequency(type) {
